    ...
    <div className="flex min-h-screen bg-bg">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-[rgba(255,255,255,0.07)]">
          <div className="w-9 h-9 bg-[var(--red)] rounded-xl flex items-center justify-center text-lg shadow-[0_0_20px_rgba(232,55,42,0.3)] flex-shrink-0">✈</div>
          <div>
            <div className="font-syne font-extrabold text-base leading-none">Visa<span className="text-[var(--red)]">AI</span></div>
        ...
        <div className="p-3 border-t border-[rgba(255,255,255,0.07)]">
          <div className="flex items-center gap-2.5 p-3 bg-bg3 rounded-lg border border-[rgba(255,255,255,0.07)] hover:border-[rgba(255,255,255,0.12)] transition-colors cursor-pointer group">
            ...
            <button onClick={() => { logout(); window.location.href = '/login' }}
              className="opacity-0 group-hover:opacity-100 text-[var(--text3)] hover:text-[var(--red)] transition-all text-sm" title="Logout">⏻</button>
          </div>
        </div>
      </motion.nav>

      {/* Main area */}
      <div className="flex-1 flex flex-col min-h-screen transition-all duration-300" style={{ marginLeft: sidebarOpen ? 260 : 0 }}>
        {/* Header */}
        <header className="h-16 bg-bg2 border-b border-[rgba(255,255,255,0.07)] flex items-center gap-4 px-6 sticky top-0 z-40">
          <button onClick={toggleSidebar} className="btn-ghost text-lg">{sidebarOpen ? '◀' : '▶'}</button>
          ...
          <button className="relative w-9 h-9 bg-bg3 border border-[rgba(255,255,255,0.07)] rounded-lg flex items-center justify-center text-base hover:border-[rgba(255,255,255,0.12)] transition-colors"
          ...
          <NavLink to="/leads">
            <button className="btn-primary text-xs py-2">+ New Lead</button>
          </NavLink>
        </header>
