      ...
      <motion.div initial={{opacity:0,scale:.95}} animate={{opacity:1,scale:1}} className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md z-10 overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100 text-xs text-indigo-700">
            ℹ️ Processing: 15 working days after appointment. {country.embassy} handles {country.name} Schengen visas.
          </div>
...
export default function WeVisaSchengenPage() {
  ...
  const [previewC,setPreviewC]       = useState(COUNTRIES[0])
    ...
    <div className="p-4 lg:p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        {COUNTRIES.map((c,i)=>(
          <motion.div key={c.name} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*.05}}
            className={`bg-white rounded-2xl border ${previewC.name===c.name?'border-indigo-400 shadow-lg':'border-gray-100 shadow-sm'} p-4 cursor-pointer hover:border-indigo-300 hover:shadow-md transition-all`}
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={e=>{e.stopPropagation();setSelCountry(c)}}
                className="text-xs px-3 py-1.5 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all">Apply</button>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <select value={previewC.name} onChange={e=>setPreviewC(COUNTRIES.find(c=>c.name===e.target.value)||COUNTRIES[0])} className={inp} style={ii}>
          ...
          <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100 mt-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-2xl">{previewC.flag}</span>
              <div className="font-bold text-gray-800">About {previewC.name}</div>
            </div>
            <p className="text-sm text-gray-500 mb-3">{previewC.info}</p>
            <div className="text-xs text-gray-400">Embassy: {previewC.embassy}</div>
            <div className="font-extrabold text-indigo-700 text-lg mt-2">₹{previewC.price.toLocaleString()}</div>
          </div>
          <button onClick={()=>setSelCountry(previewC)} className="w-full mt-4 py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow">
            Apply for {previewC.name} Visa →
          </button>
        </div>
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="font-bold text-gray-800 mb-4">📸 Appointment Booking Screenshots — {previewC.name}</div>
          <div className="bg-gray-100 rounded-2xl h-64 flex flex-col items-center justify-center text-center">
            <span className="text-5xl mb-3">{previewC.flag}</span>
            <div className="text-gray-400 font-semibold">{previewC.name} Schengen Visa Appointment</div>
            <div className="text-sm text-gray-300 mt-1">Sample screenshots will appear here</div>
            <div className="mt-3 text-xs text-gray-400 px-8">This shows sample screenshots of the {previewC.name} Schengen visa appointment booking process for agent reference.</div>
          </div>
        </div>
      </div>
